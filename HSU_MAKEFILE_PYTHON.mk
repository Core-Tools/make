...
# Strip whitespace from Nuitka configuration variables  
NUITKA_OUTPUT_NAME := $(strip $(NUITKA_OUTPUT_NAME))
NUITKA_ENTRY_POINT := $(strip $(NUITKA_ENTRY_POINT))
NUITKA_EXCLUDES_FILE := $(strip $(NUITKA_EXCLUDES_FILE))
NUITKA_EXTRA_MODULES := $(strip $(NUITKA_EXTRA_MODULES))
NUITKA_EXTRA_PACKAGES := $(strip $(NUITKA_EXTRA_PACKAGES))
NUITKA_EXTRA_FOLLOW_IMPORTS := $(strip $(NUITKA_EXTRA_FOLLOW_IMPORTS))
NUITKA_BUILD_MODE := $(strip $(NUITKA_BUILD_MODE))
...
# Read excluded packages from file (simple path resolution)
ifeq ($(PYTHON_DIR),.)
    # Single-language repo: files are at root level
    ifneq ($(wildcard $(NUITKA_EXCLUDES_FILE)),)
        NUITKA_EXCLUDE_PACKAGES := $(shell grep -v '^\#' $(NUITKA_EXCLUDES_FILE) | grep -v '^$$' | tr '\n' ' ')
    ...
    # Multi-language repo: files are in Python directory
...
# Add common data files if they exist (simple path resolution)
# Single-language repos: files at root level
# Multi-language repos: files in Python directory

# Handle nuitka_excludes.txt
ifeq ($(PYTHON_DIR),.)
    # Single-language repo: file at root level
    ifneq ($(wildcard $(NUITKA_EXCLUDES_FILE)),)
        NUITKA_COMMON_OPTS += --include-data-file=$(NUITKA_EXCLUDES_FILE)=$(NUITKA_EXCLUDES_FILE)
    endif
else
    # Multi-language repo: file in Python directory
    ifneq ($(wildcard $(PYTHON_DIR)/$(NUITKA_EXCLUDES_FILE)),)
        NUITKA_COMMON_OPTS += --include-data-file=$(PYTHON_DIR)/$(NUITKA_EXCLUDES_FILE)=$(NUITKA_EXCLUDES_FILE)
    endif
endif
...
ifeq ($(PYTHON_DIR),.)
    ...
    ifneq ($(wildcard requirements.txt),)
        NUITKA_COMMON_OPTS += --include-data-file=requirements.txt=requirements.txt
    endif
else
    # Multi-language repo: file in Python directory
    ifneq ($(wildcard $(PYTHON_DIR)/requirements.txt),)
        NUITKA_COMMON_OPTS += --include-data-file=$(PYTHON_DIR)/requirements.txt=requirements.txt
    endif
endif
